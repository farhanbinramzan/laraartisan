                                ...
                                <table class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($teams as $item)
                                            <tr>
                                                <td>{{ $item->id }}</td>
                                                <td><img src="{{ asset('images/' . $item->image) }}" alt="Image Not Found"
                                                        style="width: 80px;"></td>
                                                <td>{{ $item->listing_sequence }}</td>
                                                <td>{{ $item->name }}</td>
                                                ...
                                                @if ($item->social_accounts && is_array(json_decode($item->social_accounts, true)))
                                                    ...
                                                    <td>
                                                        @foreach ($socialAccounts as $account)
                                                            <div>{{ ucfirst($account['platform']) }}:
                                                                <a
                                                                    href="{{ $account['account'] }}">{{ $account['account'] }}</a>
                                                            </div>
                                                        @endforeach
                                                    </td>
                                                @else
                                                ...
                                                @endif
                                                <td><a href="{{ route('team.edit', $item->id) }}"
                                                        class="btn btn-primary">Edit</a></td>
                                                <td><a href="{{ route('team.delete', $item->id) }}"
